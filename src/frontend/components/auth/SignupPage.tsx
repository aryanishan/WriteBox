...
import { Mail, Lock, Eye, EyeOff, ArrowRight, PenLine, Check, X } from 'lucide-react';
      ...
      <div className="flex gap-1">
        {[0, 1, 2].map(i => (
          <div
            ...
            style={{
              backgroundColor: i < strength ? colors[strength - 1] : 'var(--color-border)',
            }}
          />
        ))}
      </div>
      <p className="text-xs" style={{ color: colors[strength - 1] || 'var(--color-text-muted)' }}>
        {password ? labels[strength - 1] || 'Too short' : ''}
      </p>
      ...
      <div className="space-y-1">
        {checks.map(check => (
          <div key={check.label} className="flex items-center gap-2 text-xs">
            ...
            <span className={check.met ? 'text-[var(--color-text-secondary)]' : 'text-[var(--color-text-muted)]'}>
              {check.label}
            </span>
          </div>
        ))}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[var(--color-bg-primary)] px-4">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -right-40 w-80 h-80 bg-[var(--color-accent)] opacity-[0.06] rounded-full blur-3xl" />
        <div className="absolute -bottom-40 -left-40 w-96 h-96 bg-[var(--color-accent)] opacity-[0.04] rounded-full blur-3xl" />
      </div>
        ...
        <div className="bg-[var(--color-surface-raised)] border border-[var(--color-border)] rounded-[var(--radius-lg)] shadow-lg overflow-hidden">
          ...
          <div className="px-8 pt-8 pb-2">
            ...
            <p className="text-sm text-[var(--color-text-tertiary)] mt-1">
              Start writing and syncing your notes everywhere
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-8 pb-8 pt-4 space-y-4">
            ...
            <div className="space-y-1.5">
              <label htmlFor="signup-email" className="block text-sm font-medium text-[var(--color-text-secondary)]">
                Email
              </label>
            ...
            {/* Password */}
            <div className="space-y-1.5">
              <label htmlFor="signup-password" className="block text-sm font-medium text-[var(--color-text-secondary)]">
                Password
              </label>
              <div className="relative">
            ...
            {/* Confirm Password */}
            <div className="space-y-1.5">
              <label htmlFor="signup-confirm" className="block text-sm font-medium text-[var(--color-text-secondary)]">
                Confirm Password
              </label>
              <div className="relative">
                <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[var(--color-text-muted)]" />
              ...
              {confirmPassword && password !== confirmPassword && (
                <p className="text-xs text-[var(--color-error)]">Passwords do not match</p>
              )}
            </div>
